...
if (!isset($_SESSION['usuario']) || $_SESSION['rol'] != 'encargado_ventas') {
...
if (isset($_POST['confirmar_pago'])) {
    ...
    $stmt->execute();
    $mensaje = "Pago confirmado. Comprobante: $comprobante";
...
if (isset($_POST['cambiar_estado'])) {
    ...
    if ($stmt->execute()) { $mensaje = "Estado actualizado."; $tipo_mensaje = "success"; }
}
...
// ========== STATS ==========
$pendientes  = $conn->query("SELECT COUNT(*) as t FROM pedidos WHERE estado='pendiente_pago'")->fetch_assoc()['t'];
$confirmados = $conn->query("SELECT COUNT(*) as t FROM pedidos WHERE estado='confirmado'")->fetch_assoc()['t'];
$en_camino   = $conn->query("SELECT COUNT(*) as t FROM pedidos WHERE estado='en_camino'")->fetch_assoc()['t'];
$ventas_hoy  = $conn->query("SELECT COALESCE(SUM(total),0) as t FROM ventas WHERE DATE(fecha)=CURDATE()")->fetch_assoc()['t'];

// ========== PEDIDOS ==========
$filtro = isset($_GET['estado']) ? $_GET['estado'] : '';
$sql = "SELECT p.*, c.nombres, c.apellidos, c.celular FROM pedidos p JOIN clientes c ON p.id_cliente = c.id";
...
<head>
    ...
    <title>Panel Ventas - El Otro</title>
...
<aside class="sidebar" id="sidebar">
    ...
    <div class="sidebar-user">
        ...
        <div class="user-info"><small>Encargado de Ventas</small><span><?php echo htmlspecialchars($_SESSION['usuario']); ?></span></div>
    </div>
    <nav class="sidebar-nav">
        <div class="nav-section">Principal</div>
        <a href="panel_ventas.php" class="nav-item active"><i class="bi bi-grid-fill"></i> Dashboard</a>
        <div class="nav-section">Operaciones</div>
        <a href="panel_ventas.php?estado=pendiente_pago" class="nav-item"><i class="bi bi-hourglass-split"></i> Pendientes pago</a>
        <a href="panel_ventas.php?estado=confirmado" class="nav-item"><i class="bi bi-check-circle"></i> Confirmados</a>
        <a href="panel_ventas.php?estado=listo_recoger" class="nav-item"><i class="bi bi-bag-check"></i> Listos recoger</a>
        <a href="panel_ventas.php?estado=en_camino" class="nav-item"><i class="bi bi-bicycle"></i> En camino</a>
        <a href="panel_ventas.php?estado=entregado" class="nav-item"><i class="bi bi-check2-all"></i> Entregados</a>
...
<div class="main-content">
    <div class="topbar">
        <div class="topbar-left">
            ...
            <span class="topbar-title">Panel de Ventas</span>
        </div>
    ...
    <div class="page-content">
        ...
        <div class="stats-grid" style="margin-bottom:24px;">
            <div class="stat-card">
                <div class="stat-icon" style="background:rgba(170,170,170,0.15); color:#aaa;"><i class="bi bi-hourglass-split"></i></div>
                <div class="stat-info"><h3><?php echo $pendientes; ?></h3><p>Pendientes pago</p></div>
            </div>
            <div class="stat-card">
                <div class="stat-icon azul"><i class="bi bi-check-circle"></i></div>
                <div class="stat-info"><h3><?php echo $confirmados; ?></h3><p>Confirmados</p></div>
            </div>
            <div class="stat-card">
                <div class="stat-icon" style="background:rgba(255,107,53,0.15); color:#ff6b35;"><i class="bi bi-bicycle"></i></div>
                <div class="stat-info"><h3><?php echo $en_camino; ?></h3><p>En camino</p></div>
            </div>
            <div class="stat-card">
                <div class="stat-icon verde"><i class="bi bi-cash-coin"></i></div>
                <div class="stat-info"><h3>Bs <?php echo number_format($ventas_hoy, 2); ?></h3><p>Ventas hoy</p></div>
            </div>
        </div>
        ...
        <div class="filtros">
            <a href="panel_ventas.php" class="btn-filtro <?php echo !$filtro ? 'active' : ''; ?>">Todos</a>
        ...
        <div class="tabla-pedidos">
            <div class="table-responsive">
                <table class="table table-borderless mb-0">
                    <thead>
                        <tr><th>#</th><th>Cliente</th><th>Total</th><th>Estado</th><th>Fecha</th><th>Acciones</th></tr>
                    </thead>
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <a href="detalle_pedido.php?id=<?php echo $p['id']; ?>" class="btn btn-sm btn-info me-1"><i class="bi bi-eye-fill"></i></a>
                                <?php if ($p['estado'] == 'pendiente_pago'): ?>
                                <form method="POST" class="d-inline">
                                    <input type="hidden" name="id" value="<?php echo $p['id']; ?>">
                                    <button type="submit" name="confirmar_pago" class="btn btn-sm btn-success me-1" onclick="return confirm('¿Confirmar pago QR?')"><i class="bi bi-check-circle-fill"></i></button>
                                </form>
                                <?php endif; ?>
                                <?php if ($p['estado'] == 'confirmado'): ?>
                                <button class="btn btn-sm btn-warning me-1" onclick="abrirRepartidor(<?php echo htmlspecialchars(json_encode($p)); ?>)"><i class="bi bi-bicycle"></i></button>
                                <?php endif; ?>
                                <?php if (!in_array($p['estado'], ['entregado', 'cancelado'])): ?>
                                <button class="btn btn-sm btn-secondary" onclick="abrirEstado(<?php echo $p['id']; ?>, '<?php echo $p['estado']; ?>')"><i class="bi bi-arrow-repeat"></i></button>
                                <?php endif; ?>
                            </td>
                        </tr>
                        <?php endwhile; ?>
